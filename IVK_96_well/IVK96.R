...
for (i in 1:7){
  g1x[i] = round(4*750/(4^i), digits = 1)
  g2x[i] = round(4*750/(4^i), digits = 1)
  g3x[i] = round(4*0.56/(4^i), digits = 4)
}
...
timeSlice = which(timesOut == 46)

plot4 = plot_ly() %>%
  #add_trace(x = g3x, y = gAve[,1,timeSlice], name = "DOTA-Ac", type = 'scatter', mode = 'lines+markers', marker = list(color=pal[1]), line = list(color=pal[1]), error_y = ~list(array = gErr[,1,timeSlice], color = pal[1])) %>%
  add_trace(x = g3num, y = gAve[,2,timeSlice], name = "IgG(#2)-DOTA-Ac", type = 'scatter', mode = 'lines+markers', marker = list(color=pal[2]), line = list(color=pal[2]), error_y = ~list(array = gErr[,2,timeSlice], color = pal[2])) %>%
  add_trace(x = g3num, y = gAve[,3,timeSlice], name = "IgG(#2)", type = 'scatter', mode = 'lines+markers', marker = list(color=pal[3]), line = list(color=pal[3]), error_y = ~list(array = gErr[,3,timeSlice], color = pal[3])) %>%
          layout(
          xaxis = list(type = "log", title = "mAb (mg/mL)", showticklabels = TRUE, showgrid = FALSE),
          yaxis = list(title = "Normalized Growth", showticklabels = TRUE, showline = TRUE, showgrid = FALSE, range = c(0, 1)),
          font = list(size = 12))
...
plot5 = plot_ly() %>%
  add_trace(x = g1num, y = gAve[,1,timeSlice], name = "DOTA-Ac", type = 'scatter', mode = 'lines+markers', marker = list(color=pal[1]), line = list(color=pal[1]), error_y = ~list(array = gErr[,1,timeSlice], color = pal[1])) %>%
  add_trace(x = g1num, y = gAve[,2,timeSlice], name = "IgG(#2)-DOTA-Ac", type = 'scatter', mode = 'lines+markers', marker = list(color=pal[2]), line = list(color=pal[2]), error_y = ~list(array = gErr[,2,timeSlice], color = pal[2])) %>%
  #add_trace(x = g3x, y = gAve[,3,timeSlice], name = "IgG(#2)", type = 'scatter', mode = 'lines+markers', marker = list(color=pal[3]), line = list(color=pal[3]), error_y = ~list(array = gErr[,3,timeSlice], color = pal[3])) %>%
  layout(
    xaxis = list(type = "log", title = "Total Daughter Activity (nCi/100 uL)", showticklabels = TRUE, showgrid = FALSE),
    yaxis = list(title = "Normalized Growth", showticklabels = TRUE, showline = TRUE, showgrid = FALSE, range = c(0, 1)),
    font = list(size = 12))
